...
if ( ! class_exists( 'lsd' ) ) :
	...
	class lsd {
		...
		public function setup() {
			 ...
			 * to change 'lsd' to the name of your theme in all the template files.
			 */
			load_theme_textdomain( 'lsd', get_template_directory() . '/languages' );
			...
			register_nav_menus( array(
				'main-menu'   => __( 'Main Menu', 'lsd' ),
				'social'  => __( 'Social Links', 'lsd' ),
			) );
			...
			add_filter( 'image_size_names_choose', 'lsd_custom_sizes', 1, 5 );

			function lsd_custom_sizes( $sizes ) {
		...
		public function scripts() {
			 ...
			 * Styles
			 */
			wp_enqueue_style( 'lsd-style', get_stylesheet_uri(), array( 'dashicons' ), false );
		...
		public function body_classes( $classes ) {
			...
			switch (get_theme_mod('lsd_ui_style')) {
			    case 'square':
			       	$classes[] = 'ui-square';
			        break;
			    case 'geometric':
			...
			switch (get_theme_mod('lsd_header_layout')) {
			    case 'left':
			        $classes[] = 'nav-left';
			        break;
			    case 'center':
			...
			switch (get_theme_mod('lsd_mobile_menu_type')) {
			    case 'top':
			        $classes[] = 'mobile-top';
			        break;
					case 'right':
			...
			if ( get_theme_mod('lsd_fixed_header') ) {
				$classes[] = 'fixed-header';
			}

			// Add a class for content layout
			switch (get_theme_mod('lsd_general_layout')) {
					case 'framed':
			...
			// Add a class for content layout
			switch (get_theme_mod('lsd_footer_layout')) {
					case 'stacked':
			...
			if( get_theme_mod('lsd_sidebar') == true ) {
